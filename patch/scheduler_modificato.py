...
# GUAG_2024 - Era comenntato con i parametri sbagliati !!!!! ho aggiunto la chiamata giusta.
            #await rt_sleep(rt_factor, rt_start, sim, world)
            await rt_sleep(sim, world)
#            
...
#    print(f'----->   rt_sleep:')
    if world.rt_factor:
        ...
        print(f'----->   rt_sleep: time_to_sleep={sleep} rt_passed={rt_passed} world.rt_factor={world.rt_factor}  sim.next_steps[0].time={ sim.next_steps[0].time}')
        if sleep > 0:
...
def rt_check(
    ...
    if rt_factor:
        ...
        if delta > 0.05: # GUAG_2024 warning solo se in totale c'è un errore di 0.05 secondi
            if rt_strict:
                raise RuntimeError(
                    f'Simulation too slow for real-time factor {rt_factor}'
                )
            else:
                last_step=sim.last_step.time
                logger.warning(
                    'Simulation too slow for real-time factor {rt_factor} - {delta}s '
                    'behind time.   rt_start={rt_start} rt_passed={rt_passed} sim.last_step.time={last_step}',
                    rt_factor=rt_factor,
                    delta=delta,
                    rt_start=rt_start,
                    rt_passed=rt_passed,
